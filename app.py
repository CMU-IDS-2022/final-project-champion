...
if page_selection == "US National Wide":
    ...
    if metric_selection=="Rental Price":
        ...
        graph_rentalprice=(background+point+text).properties(
            width=650,
            height=400,
            title="US City Average Rental Price of "+roomtype_selection+" at "+str(year_selection))
        st.write(graph_rentalprice)
    
        city_selection="New York"
        ...
        main=alt.Chart(df).mark_bar().encode(
            y=alt.Y("RoomType:O", sort='y',axis=alt.Axis(ticks=False, domain=False,offset=5,title=None,labelFontStyle='bold')),
            # y=alt.Y("CrimeType:N",sort='-x',axis=alt.Axis(title=None,)),
            x=alt.X("mean(RentalPrice)",axis=None),
            color=alt.Color("mean(RentalPrice):Q", sort='x',
        ...
        background = alt.Chart(states).mark_geoshape(
            ...
            width=650,
            height=400,
            title="US City Average Crime Score at "+str(year_selection)
        ).project('albersUsa')
        # background
        cities = alt.Chart(df).encode(
            longitude='longitude:Q',
        ...
        bars=alt.Chart(df).mark_bar().encode(
            y=alt.Y("CrimeType:N", sort='y',axis=alt.Axis(ticks=False, domain=False,offset=5,title=None,labelFontStyle='bold')),
            # y=alt.Y("CrimeType:N",sort='-x',axis=alt.Axis(title=None,)),
            x=alt.X("Rate",axis=None),
            color=alt.Color("Rate:Q", sort='x',
                            scale=alt.Scale(scheme='goldred'),
                            legend=None),   
        ).properties(
            width=140,
